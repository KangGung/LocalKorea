...
### Preview Mode (Host)
- **Purpose:** Allow hosts to validate content and layout before publishing while sharing a controlled, unpublished view for approvals.
- **Entry points:**
  - Persistent "Preview" button in the New Experience Form header and final Review & Publish step.
  - Preview links from the dashboard for drafts and unpublished updates.
- **Behavior & Layout:**
  - Opens in a responsive frame (mobile default with desktop toggle) that mirrors the Experience Details page, including media, accessibility badges, and price/availability modules.
  - Shows "Unpublished preview" banner and disables checkout/payment, replacing CTAs with "Ready to publish?" guidance.
  - Blocks indexation/sharing by default but exposes a copyable, time-bound private URL for collaborators; expires after 72 hours or when the listing is published.
- **Validation & States:**
  - Surfaces missing-required-field alerts and broken media placeholders directly in preview.
  - Error banner when preview link is expired; CTA to regenerate from dashboard or form.
  - Success toast after regenerating a private preview link; clipboard copy confirmation.
